...
int main( int argc, char** argv )
{
    ...
    for (int x = 0; x < sampleNum; x++) {
        ...
        cv::Mat rgb2 = cv::imread( "../scene0220_02/RGB/" + std::to_string(x+1)+".png");
        cv::Mat depth1 = cv::imread( "../scene0220_02/depth/" + std::to_string(x) + ".png", -1);
        cv::Mat depth2 = cv::imread( "../scene0220_02/depth/" + std::to_string(x+1) + ".png", -1);
        // 声明特征提取器与描述子提取器
