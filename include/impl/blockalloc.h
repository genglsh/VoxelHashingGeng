#ifndef DSVH_BLOCKALLOC_H
#define DSVH_BLOCKALLOC_H
...
struct BlockAllocBase {
  ...
  BlockAllocBase(int num_elems = 0)
	...
	inline Value &operator[](int32_t pointer) const {
		...
		return data[pointer];//Value是Voxelblock 是voxel的集合 每个block中有8*8*8个voxel 每个voxel中有tsdf weight rgb
	}
  ...
  int32_t allocate_n(int n) {
    ...
    // subtract link head by n 此处操作为什么不直接在device上做(这个地方他不会device上的做加减法操作)
    h_link_head -= n;
    if (h_link_head < 0) {
